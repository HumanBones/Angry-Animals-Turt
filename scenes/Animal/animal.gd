...
var _last_drag_vector: Vector2 = Vector2.ZERO
...
func _physics_process(delta):
	...
	label.text += "%.1f,%.1f" % [_drag_vector.x, _drag_vector.y]


func get_impulse() -> Vector2:
	return _drag_vector * -1 * IMPULSE_MULT
...
func play_stretch_sound() ->void:
	if(_last_drag_vector - _drag_vector).length() > 0:
...
func drag_in_limits() -> void:
	_last_drag_vector = _drag_vector
	_drag_vector.x = clampf(_drag_vector.x, DRAG_LIM_MIN.x, DRAG_LIM_MAX.x)
	_drag_vector.y = clampf(_drag_vector.y, DRAG_LIM_MIN.y, DRAG_LIM_MAX.y)
